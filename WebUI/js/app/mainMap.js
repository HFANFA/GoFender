require([
    ...
    var View = {
        ...
        _bindMapViewEvents: function () {
            ...
            $('.ViewInfo').live('click', function () {
                onclick = window.location.href = './info/index.html';
            });
        },
        ...
        _drawMapLineDetail: function (attack, posX, posY) {
            ...
            if ($frag.length === 0) {
                ...
                $frag.append(
                    '<p>时间: ' + attack['time'] + '</p>' +
                    '<p>攻击类型: ' + attack['type'] + '</p>' +
                    '<p>攻击源: ' + attack['srcIp'] + ' (' + attack['srcName'] + ')' + '</p>' +
                    '<p>攻击目标: ' + attack['destIp'] + ' (' + attack['destName'] + ')' + '</p>'
                );
        ...
        getAttacks: function () {
            var that = this;
            $.get(
                'http://127.0.0.1:8888' + '/api/atkinfo', function (data) {
