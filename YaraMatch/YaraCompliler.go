			...
			if (path.Ext(fileName) == ".yar") || (path.Ext(fileName) == ".yara") {
				...
				if err == nil {
					...
					if errRet == nil {
						...
						if debug {
							log.Fatalf("invalid rule file: %v, detail: %v", filePath, errRet)
						}
					}
				}
			}
	...
	case mode.IsRegular():
		...
		if err == nil {
			...
			if errRet == nil {
				...
				if debug {
					log.Fatalf("invalid rule file: %v, detail: %v", rulesPath, errRet)
				}
			}
		}
	}
...
func initRule(ruleFiles []string, debug bool) error {
	...
	for _, rulePath := range ruleFiles {
		...
		if err == nil {
			...
			if debug {
				log.Printf("Compiling Yara rule: %v, result: %v", rulePath, err)
			}
		}
	}
	...
	log.Printf("Init rules Done, total: %v rules, err: %v", total, err)
	return err
}
